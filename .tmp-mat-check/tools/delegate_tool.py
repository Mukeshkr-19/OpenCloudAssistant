...
def _batch_outcome(results, expected_count):
    successful = sum(entry.get("status") == "completed" for entry in results)
    all_succeeded = len(results) == expected_count and successful == expected_count
    return {
...
def delegate_task(
    ...
    def _execute_and_aggregate(*, honor_parent_interrupt: bool = True) -> dict:
        ...
        combined: Dict[str, Any] = {
            ...
            **_batch_outcome(results, n_tasks),
        }
        if live_paths:
