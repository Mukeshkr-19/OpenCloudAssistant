...
def check_browser_requirements() -> bool:
    ...
    # HERMES_OPENCLOUD_BROWSER_AVAIL_V1: require Camofox to be reachable so we
    # do not advertise browser tools that will fail at call time.
    if _is_camofox_mode():
        try:
            from tools.browser_camofox import check_camofox_available
            return bool(check_camofox_available())
        except Exception:
            return False
...
check_browser_requirements._opencloud_no_failure_grace = True  # HERMES_OPENCLOUD_BROWSER_AVAIL_V1
