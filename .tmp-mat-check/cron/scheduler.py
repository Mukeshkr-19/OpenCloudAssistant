...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    _provider_failure = any(marker in lower for marker in (
        ...
        "only supports single tool-call", "only supports single tool call",
...
def _cron_zero_match_report(date):
    ...
    Its text must match ``cron.output_contract.render_zero_match`` called with
    no coverage argument (the degraded/partial state).
    """
...
def run_job(
    ...
    try:
        ...
        if _cron_fleet_route is not None:
