...
# before returning so later task turns keep their tools. Apply after
# build_turn_context (MCP between-turns refresh rebuilds agent.tools).
_OPENCLOUD_SIDE_EFFECT_TOOLS = frozenset({
    ...
    "terminal", "run_terminal", "shell", "bash",
})
...
def _opencloud_is_conversational_greeting(text: str) -> bool:
    ...
    if not raw or len(raw) > 80:
        return False
    # Block anything that looks like a task, URL, or code: never a pure
    # conversational greeting, so tools (incl. clarify) must stay available.
    if re.search(
        r"https?://|/\S|\b(?:search|find|browse|open|run|fix|deploy|cron|install|"
        r"write|create|send|schedule|book|order|build|make|update|delete|remove|"
        r"switch|config|set|show|list|add|summarize|explain|tell|reply|check|"
        r"start|stop|enable|disable|turn|do|get)\b",
        raw,
        re.I,
    ):
        return False
    # Vocative matched by SHAPE (any short word), not a hardcoded phrase list,
    # so colloquial forms ("guys","y'all","folks","team","everyone","bro",
    # "bruh","homie","boss","chief") all classify. A greeting opener plus at
    # most one trailing light token; extra sentence content is left to the
    # task guard and is NOT forced into greeting-only mode.
    if re.fullmatch(
        r"(?i)(?:hi|hello|hey|yo|sup|wassup|what'?s\s*up|howdy|hiya|greetings|"
        r"hola|bonjour|good\s*(?:morning|afternoon|evening)|hey\s*there|cheers|"
        r"gg|oi|aye)\b(?:[\s,:.!?-]+[A-Za-z][\w'-]{0,20})?[\s,:.!?-]*",
        raw,
    ):
        return True
    # Bare short interjection / vocative ("Bro","Bruh","Homie","Boss","Chief",
    # "Yo"). Shape-based; interrogatives and task verbs are excluded above.
    if re.fullmatch(r"(?i)[A-Za-z][\w'-]{0,11}[\s,:.!?]*", raw):
        tok = raw.strip(" ,.!?").lower()
        if tok and not re.fullmatch(r"(?i)(what|how|who|why|when|where|which|whom)", tok):
            return True
    return False
...
def run_conversation(
    ...
    # HERMES_OPENCLOUD_TOOL_INTENT_V1 — after build_turn_context so between-turns
    # MCP refresh cannot re-expand side-effect tools for greeting turns.
    try:
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from agent.hermes_fleet_bridge import note_agent_success
                    note_agent_success(agent)
                except Exception as _fleet_exc:
                    logger.debug(
                        "Fleet success reporting skipped: %s",
                        type(_fleet_exc).__name__,
                    )
