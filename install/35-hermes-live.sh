...
PATCH26="$ROOT/integrations/hermes/hermes-greeting-output-contract.patch"

PATCH27="$ROOT/integrations/hermes/hermes-clarify-hardening.patch"
...
require_source() {
    ...
    test -f "$PATCH18" || {
        echo "ERROR: Hermes provider-survival patch missing" >&2
        exit 1
    }
    ...
    test -f "$PATCH24" || {
        echo "ERROR: Hermes greeting tool_choice none patch missing" >&2
        exit 1
    }
    ...
    test -f "$PATCH26" || {
        echo "ERROR: Hermes greeting output-contract patch missing" >&2
        exit 1
    }

    test -f "$PATCH27" || {
        echo "ERROR: Hermes clarify hardening patch missing" >&2
        exit 1
    }
...
validate_tree() {
    ...
    for marker in $MARKERS; do
        grep -RqsF "$marker" "$tree/agent" "$tree/tools" "$tree/plugins" "$tree/cron" "$tree/gateway" "$tree/hermes_cli" "$tree/hermes_state.py" "$tree/model_tools.py" || {
    ...
    test -f "$tree/gateway/cron_control_fast_path.py" || {
    ...
    grep -qF "HERMES_COMPRESSION_FAILSAFE_V1" "$tree/gateway/run.py" || {
        echo "ERROR: compression failsafe marker missing" >&2
        return 1
    }
    grep -qF "HERMES_P13_DIAGNOSTIC_SUPPRESSION_V1" "$tree/gateway/run.py" || {
        echo "ERROR: P13 diagnostic-suppression marker missing" >&2
        return 1
    }
...
materialize() {
    ...
    echo "HERMES_INSTALL: checking provider routing/cron survival patch"
    ...
    echo "HERMES_INSTALL: checking greeting output-contract patch"
    git -C "$out" apply --check "$PATCH26"
    git -C "$out" apply "$PATCH26"

    git -C "$out" apply --check "$PATCH27"
    git -C "$out" apply "$PATCH27"
